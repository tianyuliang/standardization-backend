...
|------|------|--------|------------|
| Phase 0 | 基础设施 | 5 | 1天 |
| Phase 1 | 基础CRUD (5接口) | 20 | 5天 |
| Phase 2 | 状态管理 (1接口) | 2 | 1天 |
| Phase 3 | 目录移动 (1接口) | 2 | 0.5天 |
| Phase 4 | 关联查询 (4接口) | 8 | 2天 |
| Phase 5 | 批量查询 (2接口) | 4 | 1天 |
| Phase 6 | 内部接口 (3接口) | 6 | 1.5天 |
| Phase 7 | 辅助接口 (2接口) | 4 | 0.5天 |
| Phase 8 | 收尾工作 | 4 | 0.5天 |
| **总计** | | **55** | **约13天** |
...
1. **100% 接口兼容性**: 所有 API 路径、参数、响应格式必须与 Java 实现一致
2. **主键例外**: rule 模块使用 BIGINT 主键（与 Java 兼容），其他模块仍使用 UUID v7
3. **表结构复用**: 直接使用 Java 的 t_rule 表结构（f_ 前缀字段）
4. **错误码映射**: Java 错误码必须准确映射到 Go 错误码
5. **MQ 消息格式**: 必须与 Java 实现的 JSON 格式完全一致
6. **部门ID 处理**: 存储完整路径，返回最后一段
7. **Mock 标记**: 所有依赖其他服务的逻辑必须用 `// TODO:` 标记
