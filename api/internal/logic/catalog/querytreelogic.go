...
import (
	...
	"github.com/jinguoxing/idrm-go-base/errorx/validator"
...
func ValidateCatalogName(name string) error {
	...
	// 使用 validator 模块进行正则校验
	err := validator.ValidateRegex(name, catalog.CatalogNamePattern)
	if err != nil {
		return fmt.Errorf("目录名称格式错误: %w", err)
	}
