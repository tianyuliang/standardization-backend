...
type GetDataElementDetailLogic struct {
	logx.Logger
	ctx    context.Context
...
// 查看数据元详情
func NewGetDataElementDetailLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetDataElementDetailLogic {
	return &GetDataElementDetailLogic{
...
func (l *GetDataElementDetailLogic) GetDataElementDetail(req *types.DataElementDetailReq) (resp *types.DataElementDetailVo, err error) {
	// Step 1: 根据type和value查询数据元
	var de *dataelement.DataElement
	if req.Type == 1 {
		// 按ID查询
		de, err = l.svcCtx.DataElementModel.FindOne(l.ctx, req.Value)
	} else if req.Type == 2 {
		// 按Code查询
		de, err = l.svcCtx.DataElementModel.FindOneByCode(l.ctx, req.Value)
	} else {
		return nil, errorx.InvalidParameter("type", "查询类型必须为1(ID)或2(Code)")
	}
